...
var idTabs, //shortcut
undefined,  //speed up
tabid = function(e){ return $(e).attr("tabid"); },
...
idTabs = $.idTabs = function(tabarea,options,data){
  ...
  // Find tabs
  items = []; //save elements
  s.tabs = tabs = $("a[tabid^=#]",tabarea); //save tabs
  tabs.each(function(){ //add items
    test = s.item(tabid(this));
  ...
  || type(s.start) == "String" && (test=tabs.filter("a[tabid=#"+s.start+"]")).length
...
idTabs.remove = function(s){
  ...
  return s.area.each(function(){
    ...
    tabs = s.tabs || $("a[tabid^=#]",this); //save tabs
    if(!tabs.length && $(this).is("a[tabid^=#]")) tabs = $(this);
    tabs.trigger(e);
  });
};
...
idTabs.find = function(e){
  ...
  $("a[tabid="+tabid(this)+"]:first",s.area).each(function(){
...
idTabs.tab = function(id){
  if(!id) return $([]);
  return $("a[tabid="+id+"]",this.tabarea);
};
