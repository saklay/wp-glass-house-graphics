...
class TwitterStatus
{
	...
	public function __construct($id = null, $count = 0) {
		...
		// constants
		if( !defined( __DIR__ ) )define( __DIR__, dirname(__FILE__) ); /* added since __DIR__ is for PHP 5.3 and above */
		...
		$this->WidgetTemplate = 
			...
			'<div class="twitterballoon">{TWEETS}</div>' . 
			'</div>';
			
		// default tweet template
		$this->TweetTemplate = 
			'<span class="twittertop"></span>' .
			'<span>{text} <em>{created_at}</em></span>' .
			'<span class="twitterbottom"></span>';
